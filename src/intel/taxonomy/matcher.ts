...
function matchTrack(vocab: TrackVocabulary, fields: readonly TextField[]): TrackMatch {
  ...
  for (const { field, text } of fields) {
    ...
    for (const term of vocab.positive) {
      ...
      ;(isGeneric ? generic : specific).add(term)
    ...
    for (const term of vocab.negative) {
      if (findOccurrences(text, term).length > 0) negatives.add(term)
    }
  }

  return {
    track: vocab.key,
    confidence: confidenceFrom(specific.size, generic.size, negatives.size),
    snippets,
    specificTerms: [...specific].sort(),
    negativeTerms: [...negatives].sort(),
  }
}
